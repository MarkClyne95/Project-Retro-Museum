...
namespace ScottEwing{
    /*[Serializable]
    public class S_Health : IActorComponent ,ITakesDamage{
    ...
    public class S_Health : MonoBehaviour, ITakesDamage{
        ...
        public void TakeDamage(int damage, RaycastHit hit) {
            ...
            if (_currentHealth > 0 ) {
                _sActorController.BroadcastTakeDamageEvent(damage, _currentHealth, hit);
            }
            else if (_currentHealth <= 0) {
                bool isGibbed = _canBeGibbed && damage >= _gibDamageThreshold;
                _currentHealth = 0;
                gameObject.layer = Layers.DeadValue();
                _sActorController.BroadcastDeathEvent(damage, _currentHealth, hit, isGibbed);
            }
        }
    }
